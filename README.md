...
- **任务状态**：待办 / 进行中 / 已完成 / 已取消。左键在圆点上循环前三态；右键圆点可直接选「已取消」（与完成一样沉底、记结束时间、可归档）
- **拖动排序**：按住任务左侧手柄拖动，置顶区与普通任务区各自排序
- **任务置顶**：重要任务固定在分组顶部
...
- **循环任务**：每完成一次会在已完成区留下快照，原任务回到待办并累计次数
- **开机自启**：可选开机自动启动
- **窗口置顶**：可选始终显示在最前
- **托盘常驻**：关闭窗口后进程仍在托盘运行（提醒、快捷键有效）；右键托盘图标选「退出」才彻底关闭
- **4K 友好**：UI 针对高分辨率屏幕优化
...
未签名的安装包被 Gatekeeper 隔离后会显示这个提示，并不是文件损坏。把 `.app` 拖到「应用程序」后，在终端执行：

```bash
xattr -cr /Applications/iamnote.app
```

或：打开「系统设置 → 隐私与安全性」，点「仍要打开」。根治需要 Apple 开发者证书签名并公证。
